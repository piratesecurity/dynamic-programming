...
void budget_plan(int *party_prices,int *party_funs,int num_parties,int budget)
				...
				if (party_prices[i-1]<=j)
					{
						...
						if (with_i>solutions[i-1][j])
							{
								solutions[i][j]=with_i;
								sol_weights[i][j]=party_prices[i-1]+sol_weights[i-1][j-party_prices[i-1]];
							}
						else
							{
	...
	//if (global!=0) printf("\n");
	//global++;
	cout <<sol_weights[num_parties][budget]<<" "<< solutions[num_parties][budget]<< endl;	
	}
